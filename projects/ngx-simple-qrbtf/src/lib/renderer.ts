...
export function defaultViewBox<T extends DefaultRendererOptions>({ qrcode }: T) {
  ...
  const xy = 0; // String(-nCount / 5);
  const widthHeight = nCount; // String(nCount + nCount / 5 * 2);

  return `${xy} ${xy} ${widthHeight} ${widthHeight}`;
}
...
export function defaultDrawIcon<T extends DefaultRendererOptions>({ qrcode, icon }: T) {
  ...
  if (icon) {
    ...
    if (icon && iconEnabled) {
      ...
      pointList.push(
        `<path d='${sq25}' stroke='#FFF' stroke-width='${100 / iconSize * 1 }' fill='#FFF' transform='${transform}' />`
      );
      pointList.push(`
        <g>
          <defs>
            <path id='${'defs-path' + randomIdDefs }' d='${sq25}' fill='#FFF' transform='${transform} ' />
          </defs>
...
export function createRenderer<T>(rendererProps: Renderer<T>) {
  ...
  return (props?: DeepPartial<RendererOptions<T>>) => {
    ...
    const template = [];
    const viewBox = renderer.getViewBox ? renderer.getViewBox(newProps) : '';
    template.push(
