        ...
        if (qrcode.isDark(x, y) && typeTable[x][y] === QRPointType.POS_CENTER) {
          ...
          if (
            ...
            if (
              ...
              g1.push(
                `<circle opacity='${opacity}' cx='${x + 1 + 0.5}' cy='${y + 1 + 0.5
              ...
              if (qrcode.isDark(x + 1, y + 1)) {
                g1.push(
                  `<circle opacity='${opacity}' r='${0.5 * rand(0.5, 1)}' fill='${otherColor}' cx='${x + 1 + 0.5}' cy='${y + 1 + 0.5}'/>`
                );
              }
          ...
          if (x < nCount - 1 && y < nCount - 1) {
            if (
              ...
              g1.push(
                `<circle opacity='${opacity}' cx='${x + 1}' cy='${y + 1}' r='${Math.sqrt( 1 / 2 )}' fill='#FFFFFF' stroke='${otherColor}' stroke-width='${rand( 0.33, 0.6 )}' />`
              );
              for (let i = 0; i < 2; i++) {
                for (let j = 0; j < 2; j++) {
          ...
          if (available[x][y] && y < nCount - 1) {
            if (qrcode.isDark(x, y) && qrcode.isDark(x, y + 1)) {
              pointList.push(
                `<circle opacity='${opacity}' cx='${x + 0.5}' cy='${y + 1}' r='${0.5 * rand(0.95, 1.05)}' fill='#FFFFFF' stroke='${otherColor}' stroke-width='${rand(0.36, 0.4)}' />`
              );
              available[x][y] = false;
              available[x][y + 1] = false;
            }
          ...
          if (available[x][y] && x < nCount - 1) {
            if (qrcode.isDark(x, y) && qrcode.isDark(x + 1, y)) {
              pointList.push(
                `<circle opacity='${opacity}' cx='${x + 1}' cy='${y + 0.5}' r='${0.5 * rand(0.95, 1.05)}' fill='#FFFFFF' stroke='${otherColor}' stroke-width='${rand(0.36,0.4)}' />`
              );
              available[x][y] = false;
              available[x + 1][y] = false;
            }
          ...
          if (available[x][y]) {
            if (qrcode.isDark(x, y)) {
              pointList.push(
                `<circle opacity='${opacity}' r='${0.5 * rand(0.5, 1)}' fill='${otherColor}' cx='${x + 0.5}' cy='${y + 0.5}'/>`
              );
            } else if (typeTable[x][y] === QRPointType.DATA) {
              if (rand(0, 1) > 0.85) {
                g2.push(
                  `<circle opacity='${opacity}' r='${0.5 * rand(0.85, 1.3)}' fill='#FFFFFF' stroke='${otherColor}' stroke-width='${rand(0.15,0.33)}' cx='${x + 0.5}' cy='${y + 0.5}'/>`
                );
              }
            }
          }
